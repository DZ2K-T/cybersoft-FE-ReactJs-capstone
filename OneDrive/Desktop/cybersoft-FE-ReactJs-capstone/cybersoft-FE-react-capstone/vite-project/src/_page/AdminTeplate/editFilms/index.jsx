...
import { formEdit } from "./duck/reducer";
import { useParams } from "react-router";
import { getInforFilm } from "./duck/inforFilm";
import React from "react";

const FilmComponent = ({ id }) => {};

export default function Editfims() {
  ...
  const [edit, setEdit] = useState(null); // Khởi tạo với giá trị null hoặc đối tượng rỗng
  const [resultInfoFlim, setResultInfoFlim] = useState(null);

  useEffect(() => {
    dispatch(getInforFilm(id))
      .then((result) => {
        const resultData = result.payload;
        setResultInfoFlim(resultData); // Lưu trữ dữ liệu vào state
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if (resultInfoFlim) {
      const ngayKhoiChieu = new Date(resultInfoFlim.ngayKhoiChieu);
      const formattedNgayKhoiChieu = `${ngayKhoiChieu.getFullYear()}-${(
        ngayKhoiChieu.getMonth() + 1
      )
        .toString()
        .padStart(2, "0")}-${ngayKhoiChieu
        .getDate()
        .toString()
        .padStart(2, "0")}`;

      console.log(formattedNgayKhoiChieu);

      setEdit({
        maPhim: resultInfoFlim.maPhim,
        tenPhim: resultInfoFlim.tenPhim,
        trailer: resultInfoFlim.trailer,
        moTa: resultInfoFlim.moTa,
        ngayKhoiChieu: formattedNgayKhoiChieu,
        sapChieu: resultInfoFlim.sapChieu,
        dangChieu: resultInfoFlim.dangChieu,
        hot: resultInfoFlim.hot,
        danhGia: resultInfoFlim.danhGia,
        hinhAnh: resultInfoFlim.hinhAnh,
      });
    }
  }, [resultInfoFlim]); // Cập nhật khi resultInfoFlim thay đổi

  if (!edit) {
    return <div>Loading...</div>; // Hiển thị loading khi chưa có thông tin
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (!edit.tenPhim || !edit.trailer || !edit.moTa) {
      ...
      <div className="p-4 sm:ml-64">
        <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
          <div className="sm:rounded-lg pl-2 mb-3 ">
            <h1 className="font-bold mb-3">Thêm mới phim</h1>
          </div>
  ...
  //   FormDataID.append(key, info[key]);
  // });
  // dispatch(getInforFilm(id));
  // const resultInfoFlim = "";

  // dispatch(getInforFilm(id))
  //   .then((result) => {
  //     const resultInfoFlim = result.payload;
  //     console.log(resultInfoFlim);
  //   })
  //   .catch((error) => {});
  ...
  //   trailer: resultInfoFlim.trailer,
  //   moTa: resultInfoFlim.moTa,
  //   ngayKhoiChieu: resultInfoFlim.ngayKhoiChieu,
  //   sapChieu: resultInfoFlim.sapChieu,
  //   dangChieu: resultInfoFlim.dangChieu,
  //   hot: resultInfoFlim.hot,
  //   danhGia: resultInfoFlim.danhGia,
